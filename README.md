# safari-yomiage-test / YomiagePlayer

iOS の「ページの読み上げ」相当を**自前**で実装したサンプルアプリです。
Safari 純正の読み上げではシークバーで位置指定ができなくなったため、
`AVSpeechSynthesizer` をベースに「**シークバーによる位置指定**」と
「**バックグラウンド再生 / ロック画面コントロール**」を実現する検証用プロジェクトです。

## できること

- テキストを貼り付けて日本語で読み上げ
- アプリ内のシークバーで**好きな位置から読み上げ再開**
- 早送り / 巻き戻し（15秒）・読み上げ速度の調整
- 使用する音声（ボイス）の選択（日本語の高品質音声を既定で自動選択）
- テキストの取り込み：**クリップボードからペースト** / **ファイル（.txt / .rtf）から読み込み**

### Google ドキュメントの文章を読み上げる

- **ペースト**：Google Docs で本文を全選択してコピー → アプリの「ペーストして読み込む」をタップ。
- **ファイル**：iPhone の Google Docs アプリからは **PDF / DOCX** で書き出せます
  （メニュー > 共有とエクスポート > コピーを送信 > PDF または Word）。ファイルアプリ等に保存 →
  アプリの「ファイルから読み込む」で選択。`.txt` / `.rtf` も読み込めます。
  - PDF は PDFKit でテキスト抽出（推奨・確実）。
  - DOCX は zip を展開して本文を抽出（自作・ベストエフォート）。レイアウトの複雑な文書では
    一部うまく取れない場合があります。その場合は PDF を試してください。
- **共有シート（Share Extension）**：Safari などでテキストを選択 → 共有 → 「読み上げに送る」を選ぶと、
  そのテキストがクリップボードに入ります。本体アプリを開いて「ペーストして読み込む」を押すと読み上げできます。
  （App Group を使わない簡易版のため、受け渡しはクリップボード経由です）

### Share Extension（共有シート）のビルド設定

このプロジェクトには本体アプリ（`YomiagePlayer`）に加えて、共有拡張ターゲット `ShareExtension` が含まれます。

- 実機ビルドには **両方のターゲット**に署名（Team）が必要です。
  Xcode の各ターゲット > Signing & Capabilities で同じ Apple ID（Team）を設定してください。
- Bundle Identifier は次の親子関係になっている必要があります（重複時は両方を一意な値へ変更）:
  - 本体: `com.example.YomiagePlayer`
  - 拡張: `com.example.YomiagePlayer.ShareExtension`
- 無料アカウントでも動作する想定です（App Group は使用していません）。
- **バックグラウンド再生**（`AVAudioSession` の `.playback` / `.spokenAudio`）
- **ロック画面・コントロールセンター**での再生/一時停止/スキップ
- **ロック画面のシークバー（スクラブ）**での位置指定
  （`MPRemoteCommandCenter.changePlaybackPositionCommand`）

## 仕組みのポイント

- 本文を**文単位**（。！？改行などで区切り）に分割し、1文ずつ読み上げる。
  これにより「途中の任意位置から再開」＝シークが可能になります。
- `willSpeakRangeOfSpeechString` で読み上げ位置を追従し、進捗（0〜1）を更新。
- TTS は本来「尺」を持たないため、文字数 × 想定秒数で**擬似的な総時間・経過時間**を
  作り、Now Playing（ロック画面）のシークバーへ反映しています。

### 音声（ボイス）について

- Safari の「ページの読み上げを聞く」は、**設定 > アクセシビリティ > 読み上げコンテンツ > 声**
  で選択中のシステム音声を使います。アプリ側からは「どの音声が選択中か」を取得する公開 API は
  ないため、**自動で完全一致させることはできません**。
- 代わりに、端末にある音声を一覧表示し、同じ音声を選べるようにしています
  （`AVSpeechSynthesisVoice.speechVoices()`）。既定では日本語で最も高品質な音声を選びます。
- Safari と同じにしたい場合は、上記の設定で使っている音声（Siri / 高品質 / プレミアム）を
  **先にダウンロード**しておくと、アプリの一覧にも現れて選択できます。
- 注意: **Siri 音声**は一覧に出ても、サードパーティアプリでは標準音声へフォールバックする
  ことがあります（iOS のバージョン依存）。

主要ファイル:

| ファイル | 役割 |
|---|---|
| `YomiagePlayer/SpeechManager.swift` | 読み上げ・オーディオセッション・リモートコマンド・Now Playing |
| `YomiagePlayer/ContentView.swift`   | UI（テキスト入力・シークバー・再生コントロール・速度） |
| `YomiagePlayer/YomiagePlayerApp.swift` | アプリのエントリポイント |
| `Info.plist` | `UIBackgroundModes: audio`（バックグラウンド再生に必須） |

## ビルドと実行（個人検証）

1. `YomiagePlayer.xcodeproj` を Xcode 16 以降で開く
2. ターゲット `YomiagePlayer` を選択
3. **Signing & Capabilities** で自分の Apple ID（Team）を設定
   - `PRODUCT_BUNDLE_IDENTIFIER` が他と重複する場合は
     `com.example.YomiagePlayer` を自分用に変更してください
4. 実機（iPhone）を接続して Run
   - バックグラウンド再生・ロック画面コントロールは**実機**でのみ確認できます
     （シミュレータではロック画面の挙動は確認できません）

> 注意: バックグラウンド再生には `Info.plist` の `UIBackgroundModes = audio` が必須です。
> Xcode の Capabilities から「Background Modes > Audio, AirPlay, and Picture in Picture」を
> 有効にしても同じ設定になります。

## 動作確認の手順

1. アプリを起動 → サンプル文が読み込まれる
2. 再生ボタンで読み上げ開始
3. シークバーをドラッグして離す → その位置から読み上げ再開
4. 端末をロック → ロック画面のメディアコントロールに表示
5. ロック画面のシークバーをドラッグ → 位置が変わることを確認
